...
public class Tank extends GameObj  {
    ...
    public void draw(ImageObserver obs, Graphics2D g) {
        ...
        if(health>0&&coolDown==0&&life>0){
        	...
        	if(!tankgame.p1.isRespawning()&&!tankgame.p2.isRespawning()){
        		if (tankgame.p1.collision(tankgame.p2.x, tankgame.p2.y, tankgame.p2.width, tankgame.p2.height)){
        			if(tankgame.p1.x>x){
        				tankgame.p1.x+=speed*5;
        				tankgame.p2.x-=speed*5;
        			}else if(tankgame.p1.x<(this.x)){
        				tankgame.p1.x-=speed*5;
        				tankgame.p2.x+=speed*5;
        			}if(tankgame.p1.y>this.y){
        				tankgame.p1.y+=speed*5;
        				tankgame.p2.y-=speed*5;
    		...
    		addExplosion(tankgame.getTankGame().getExplosionImg(),x,y);
    		coolDown = 180;
    		powerUp = false;
    		if(--life>=0) health = 4;
    		boom = false;
    ...
    public void shoot(Tank p){
		...
		if(powerUp){
	    	tankgame.getTankGame().getBullets().add(new TankBullet(tankgame.getTankGame().getStrongBulletImg(), speed*3, this, 0, 2));

		}else{
	    	tankgame.getTankGame().getBullets().add(new TankBullet(tankgame.getTankGame().getWeakBulletImg(), speed*2, this, 0, 1));
		}
